...
- The first dashboard is a password-gated FastAPI surface at `/dashboard` with JSON at `/api/dashboard/overview`. Public Cloudflare routing is allowed only after HTTP Basic auth is configured through `HEALTH_DASHBOARD_USER` and `HEALTH_DASHBOARD_PASSWORD` in `.env`.
