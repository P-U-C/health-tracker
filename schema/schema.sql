...
CREATE TABLE IF NOT EXISTS resting_hr        (date DATE PRIMARY KEY, resting_hr INTEGER);
CREATE TABLE IF NOT EXISTS vo2max            (date DATE PRIMARY KEY, vo2max DOUBLE);
CREATE TABLE IF NOT EXISTS hrv_samples       (ts TIMESTAMP, hrv_ms DOUBLE, source TEXT DEFAULT 'apple_watch', PRIMARY KEY (ts, source));
CREATE TABLE IF NOT EXISTS spo2_samples      (ts TIMESTAMP PRIMARY KEY, spo2_pct DOUBLE);
CREATE TABLE IF NOT EXISTS hr_hourly         (hour TIMESTAMP PRIMARY KEY, hr_avg INTEGER, hr_min INTEGER, hr_max INTEGER, samples INTEGER);
CREATE TABLE IF NOT EXISTS activity_rings    (date DATE PRIMARY KEY, active_cal INTEGER, active_goal INTEGER, exercise_min INTEGER, exercise_goal INTEGER, stand_hrs INTEGER, stand_goal INTEGER);
CREATE TABLE IF NOT EXISTS daily_metrics (
  date DATE PRIMARY KEY, active_cal DOUBLE, basal_cal DOUBLE, steps DOUBLE, distance_km DOUBLE, flights DOUBLE,
...
CREATE TABLE IF NOT EXISTS workouts (
  workout_id TEXT PRIMARY KEY, start_ts TIMESTAMP, type TEXT, duration_min DOUBLE, distance_km DOUBLE, calories INTEGER,
  hr_avg INTEGER, hr_min INTEGER, hr_max INTEGER, source TEXT,
  trimp DOUBLE, srpe_load DOUBLE, session_rpe DOUBLE);         -- computed / logged
CREATE TABLE IF NOT EXISTS body_comp_samples (date DATE, type TEXT, value DOUBLE, source TEXT, PRIMARY KEY (date, type, source));
CREATE TABLE IF NOT EXISTS sleep_segments   (start_ts TIMESTAMP, end_ts TIMESTAMP, value TEXT, source TEXT, PRIMARY KEY (start_ts, end_ts, value, source));
CREATE TABLE IF NOT EXISTS nutrition_daily  (date DATE PRIMARY KEY, calories DOUBLE, protein_g DOUBLE, carbs_g DOUBLE, fat_g DOUBLE, fiber_g DOUBLE, sodium_mg DOUBLE, potassium_mg DOUBLE, notes TEXT);
...
CREATE TABLE IF NOT EXISTS dexa_scans (
  ...
  almi_kg_m2 DOUBLE, almi_yn_pctile INTEGER, almi_am_pctile INTEGER, lmi_kg_m2 DOUBLE, fmi_kg_m2 DOUBLE,
...
CREATE TABLE IF NOT EXISTS dexa_regional (scan_date DATE, region TEXT, fat_kg DOUBLE, lean_bmc_kg DOUBLE, total_kg DOUBLE, pct_fat DOUBLE, yn_pctile INTEGER, am_pctile INTEGER, PRIMARY KEY (scan_date, region));
